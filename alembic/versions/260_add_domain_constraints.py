...
All operations are idempotent - checks table/column existence before applying.
...
# (table, constraint_name, expression, required_columns)
# required_columns: columns that must exist for the constraint to be valid
CONSTRAINTS = [
...
def upgrade() -> None:
    conn = op.get_bind()
    for table, name, expr, required_cols in CONSTRAINTS:
        if not _table_exists(conn, table):
            continue
        if not all(_column_exists(conn, table, col) for col in required_cols):
            continue
...
def downgrade() -> None:
    for table, name, _expr, _cols in reversed(CONSTRAINTS):
        op.execute(

...
- **W1 Security — Multi-Tenant-Korrektheit 1a (Migration 268)**:
  - `business_entities.company_id` ergänzt (fehlte in Modell **und** DB, obwohl `GET /entities/{id}` + 10+ Services sie referenzieren → Einzel-GET war HTTP-500-broken). Listen-Endpoints `GET /entities`, `/entities/customers`, `/entities/suppliers` filtern jetzt mandantensicher (eigene + `company_id IS NULL`) → CWE-639-Cross-Tenant-Leak geschlossen.
  - `MultipleResultsFound`-500er (Bug ②③ aus 0c) behoben: `get_user_current_company`/`get_user_company_id` lösen mehrere `is_current=True` deterministisch auf (neueste Mitgliedschaft); Partial-Unique-Index `uq_user_companies_one_current` als dauerhafter Schutz; einmaliger Dedupe in Migration 268. `require_company` ohne `X-Company-ID` → 400 statt 500.
  - **get_db-Engine-Konsolidierung**: `app/api/dependencies.get_db` baute eine **eigene** Engine parallel zu `app.db.database` → RLS-Kontext (`SET LOCAL` in `require_company`) erreichte die Endpoint-Session nie + zwei Pools. Jetzt identisches Callable (`get_db_session`).
  - Banking `user_id` nullable (`bank_accounts`/`bank_imports`/`dunning_records`): company-scoped Services (`AccountService`/`ImportService`/`DunningService`) legen ohne `user_id` an — NOT NULL ließ jede Konto-Anlage scheitern (beim Import als falscher „Datei bereits importiert"-Fehler maskiert). Aufgedeckt durch die reaktivierten Banking-Multi-Tenant-Tests (W0-conftest-Fix).
- **W1 Pipeline-Robustheit**: OCR-OOM-Endpfad setzt Dokument-Status FAILED (kein ewiges „pending"); fehlgeschlagenes Task-Enqueue beim Upload → Status „uploaded"; Celery-Idempotenz-Claim via `ProcessingJob` + `INSERT … ON CONFLICT DO NOTHING` (doppelte Zustellung skippt, Retry setzt fort, verwaiste Claims übernommen); CUDA-OOM zentral (`app/core/gpu_errors.py`) + `backend_manager` zieht bei OOM das CPU-Backend (Surya) vor; `pipeline_task_failed` mit `exc_info`.
- **W2 Test-Harness**: Schemathesis-API-Fuzzing (`make api-fuzz`, `scripts/run_schemathesis.sh`, `.github/workflows/api-fuzz.yml` PR+nightly, anfangs non-blocking) — erster Lauf fand **10 reproduzierbare 5xx-Bugs** (`docs/qa-reports/2026-06-10-schemathesis-baseline.md`). Playwright-MCP-QA-Playbook (`.claude/Docs/Testing/QA_AGENT_PLAYBOOK.md`, J1–J6) + `/test-webapp`-Preconditions.
- **W3 Pilot-Vertrauens-Loop** (Frontend + Backend): **Import-Live-Status** `GET /imports/runs` (Batch-Aggregation; Bug `min(uuid)` gefixt) + `ImportRunsPanel` mit Auto-Polling. **DATEV-Kontierungs-Validator** vor Export (`preview_export.validation_results`, `ExportValidationSummary`, Export-Button erst nach Vorprüfung). **Filing-Vorschlag nach Upload** `POST /automation/filing-suggestions/{id}/accept` via `bulk_move_category` (umgeht das latent kaputte `auto_file_document`, das nicht-existente `Document.category`/`folder_id` setzte) + `FilingSuggestionCard`. **Lexware**-Konflikt-Details im Erfolgs-Report (`ImportConflictPreview`).
- **W0 Fundament**: `feature/offensive-2026-06` (20 Commits) nach master; Worktree-/Branch-Cleanup; `ocr-performance` archiviert (Tag `archive/ocr-performance-2026-03`); banking-conftest-`db`-Fixture (17 rote Tests → grün/skip).
- **Tests**: W1 885 passed/2 strict-xfail (PaymentService noch user-scoped = Follow-up); W3 8 Pytest + 16 Vitest. Migration 268 2× idempotent. Hinweis: Vitest auf Windows nur Suite-weise stabil.

### Added — Mocks→echt Phase 1 (`feature/mocks-to-real-p1`, 2026-06-04)
- **M13** Echter Cashflow-Backtest: `cashflow_prediction_service.get_prediction_metrics` nutzt gespeicherte Vorhersagen (`PredictionFeedbackRecord`: predicted/actual/was_accurate) fuer echte Genauigkeit (accuracy/MAE); Schaetzung (`is_estimated=True`) nur noch als Fallback ohne Daten. Keine Migration (Tabelle existiert + wird befuellt).
- **M17** BPMN-Signal-Resume: `process_execution_service.signal` setzt wartende (geparkte) Nicht-Timer-Catch-/Boundary-Events der Instanz fort (Token-Konsum + `_continue_flow`) statt das Signal nur zu protokollieren. Offen (ehrlich, eigenes Feature): per-Signal-Namen-Matching (braucht Schema-Erweiterung) + Call-Activity-Sub-Instanz.
...
### Fixed — Offene Sachen: Approval/Fraud Real-Bugs (`feature/mocks-to-real-p1` → master `2d054889`, 2026-06-04)
- **Approval Auto-Approval** (Laufzeit-Crashes, nie funktioniert): `auto_approval_service` schrieb in reserviertes SQLAlchemy-`metadata=` statt Spalte `request_metadata=` (Metadaten nie persistiert) und setzte nicht existentes `completed_at=` → `resolved_at=`; Rate-Limit-/Statistik-Queries `ApprovalRequest.metadata[...].astext` → `request_metadata[...].as_string()` (`.astext` JSONB-only; `CrossDBJSON` hat nur `.as_string()`/`.as_boolean()`, cross-DB-sicher). Gleicher Fix in `api/v1/approvals.py`. (`4f49aaed`)
- **finanzki Fraud Roh-SQL** (schema- + **PG-EXPLAIN-verifiziert** gegen echte DB): `d.doc_type`→`d.document_type`, `d.entity_id`→`d.business_entity_id`, `it.total_amount`→`it.amount` (CTE-Ausgabespalten via `AS` stabil); `it.entity_id` korrekt belassen. Per EXPLAIN zusaetzlich entdeckt + behoben: `be.company_id` existiert nicht (BusinessEntity global, Scope via `company_presence`) → redundanter Filter entfernt. Alle 3 Queries liefern QUERY PLAN. (`de2f7f2a`, `5dcf0bc0`, `cf7565dd`, `96c22c7c`)
- **Umlaut-robuste BEC-Erkennung**: `fraud_ml_model` normalisiert Umlaute vor dem Matching → transliterierte Betrugstexte werden jetzt erkannt. (`de2f7f2a`)
- **2 Import-Bugs** (tote Module): `ocr_feedback.py` Pydantic-v2 `regex=`→`pattern=`; `rag/chat_rest.py` `get_chat_service`-Importpfad. (`b57870bd`)
- **Test-Drift**: Approval- + Fraud-Tests an aktuelles Verhalten angepasst (157 betroffene Unit-Tests gruen).
...
### Fixed — G3: Frontend Mocks → echt (Remediation 2026-06-03)

Strom **G3** der Remediation 2026-06-03: Mehrere Frontend-Views zeigten erfundene Mock-/Zufallsdaten (`Math.random`, `generateMock*`) als echt an oder taeuschten persistierende Aktionen vor. Quelle: `.claude/reviews/2026-06-03/MOCK_DATA_REGISTER.md` (M18–M23). Alle betroffenen Render-Pfade liefern jetzt ehrliche Empty-States statt erfundener Daten. Nur `frontend/src/**` geaendert. Verifikation: `tsc --noEmit` sauber (kein `any`), ESLint auf Zieldateien sauber, Knowledge-Graph-Tests 18/18 gruen (inkl. neuer Empty-State-Tests), `grep generateMock|Math.random` in allen 9 Zieldateien = 0 Treffer. Commit `2f9c2890`.

- **M18** `frontend/src/features/knowledge-graph/views/{RiskNetworkView,FinancialChainView,DocumentFamilyView}.tsx`: `generateMock*` + Hilfsdaten (`seededRandom`, `GERMAN_COMPANIES`, `COMMUNITY_*`) und ungenutzte lokale Hooks entfernt; `useMemo`-Fallback auf leere Strukturen statt Mock; `mockData` → `networkData`. Leere echte API-Daten greifen jetzt auf die vorhandenen Empty-State-Cards. Tests (`KnowledgeGraphViews.test.tsx`) auf echte `data`-Objekte umgestellt + je View ein Empty-State-Test ergaenzt.
- **M19** `frontend/src/app/routes/streckengeschaeft.validierung.tsx`: hardcodiertes `mockData`-Array entfernt; laedt echte Klassifikationen via `useDropShipmentList({isConfirmed:false})`, ehrliches Mapping (confidence=`confidenceScore*100`, vatId aus `parties[]`, `vatIdValid` konservativ `null`); Approve → `useConfirmClassification`, Reject → `useOverrideClassification('domestic')` (Backend hat keinen Reject-Status — Folgepunkt G1); Toast nur in `onSuccess`; Loading/Error/EmptyState. Keine PII in Logs.
- **M20** `frontend/src/features/reports/api/report-data-api.ts` + 3 Report-Views: `_getFallbackData` + alle `Math.random`-Fallbacks entfernt; neue typisierte `ReportDataUnavailableError(templateId)` statt synthetischer Daten; `logger.warn` nur mit `templateId` (kein PII); Views zeigen Empty-State „Berichtsdaten derzeit nicht verfuegbar".
- **M21** `frontend/src/features/import-wizard/{api/wizard-api,components/ImportWizard}.tsx`: 404-Fake-Vorschau ({itemCount:0, …}) entfernt; 404 als `WizardApiError(statusCode:404)` durchgereicht; Vorschau-Komponente zeigt echten Empty-State „Vorschau noch nicht verfuegbar".
- **M22** `frontend/src/features/ablage/components/StatusChangeDropdown.tsx`: `STATUS_OPTIONS` um `supported` erweitert; nicht unterstuetzte Status disabled mit Hinweis „noch nicht verfuegbar" + no-op onClick; `onSuccess()` NUR nach erfolgreicher `bulkMarkAsPaid`-Mutation (vorher faelschlich immer aufgerufen).
- **M23** `frontend/src/features/job-queue/components/charts/{SuccessRateChart,QueueLengthChart,JobThroughputChart}.tsx` + `tabs/OverviewTab.tsx`: `generateMockData()` entfernt, `data ?? []`; Empty-State „Keine Daten fuer den gewaehlten Zeitraum"; `OverviewTab` reicht echte Daten durch, Charts ohne 24h-Verlaufs-Endpoint bleiben ehrlich leer (Folgepunkt G1).
- **Folgepunkt #8** `frontend/src/lib/api/client.ts`: Token-sessionStorage → httpOnly-Cookie nur dokumentiert (Cross-Stream-Abhaengigkeit zu G1/G2), KEIN funktionaler Code-Change.

...
- **Cross-Stream aufgedeckt** (app/**, out of scope): 🔴 `Folder.permissions` Ambiguous-FK bricht `configure_mappers()` → blockiert ALLE ORM-instanziierenden Tests (auch `test_rls_context`/`test_cash_isolation`); `validation_queue_service.assign_to_editor` nutzt nicht existentes `User.company_id`; `get_trend_data` → `TrendResponse(data=...)` Schema-Mismatch; Entity-Endpoints ohne company_id-Filter; weasyprint-Service braucht `except (ImportError, OSError)`; `fail_under=90` unrealistisch.

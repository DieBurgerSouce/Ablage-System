...
**Umsetzungsstand:** Punkte 1 + 3 sind als **Migration 273** vorbereitet (`alembic/versions/273_rls_restrunde_invoices_approvals_bypass.py`, exakt 272er-Muster; verifiziert via Offline-SQL up+down, `alembic heads` = 273 single, ROLLBACK-Probelauf aller 5 ALTERs gegen die Live-DB, 0 NULL-company-Zeilen als Vorbedingung gemessen). **Anwendung = GATE**: nächster Backend-Neustart (M-08-Auto-Migration) oder bewusst `alembic upgrade head`; danach Verifikation nach Punkt 4. **Punkt 2 (`documents_insert`-Verschärfung im 3-Policy-Verbund) bleibt bewusst offen** — eigenes Design-Paket mit eigenen Regressionstests (Worker-INSERT via Bypass, API-Upload via Company-Kontext, kontextlos abgelehnt).

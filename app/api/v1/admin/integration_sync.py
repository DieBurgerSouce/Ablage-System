...
async def create_datev_writeback(
    ...
    batch = await service.create_batch(current_user.company_id, current_user.id, body.kontenrahmen)
...
async def list_datev_writeback(limit: int = Query(20, ge=1, le=100),
    ...
    from app.services.datev.datev_writeback_service import DATEVWritebackService
    service = DATEVWritebackService(db)
    batches = await service.list_batches(str(current_user.company_id), limit)
    return [DATEVBatchResponse(
...
async def create_lexware_export(
    ...
    if export_type == "customers":
        job = await service.export_customers(current_user.company_id)
    elif export_type == "suppliers":
        job = await service.export_suppliers(current_user.company_id)
    elif export_type == "payments":
        job = await service.export_payment_status(current_user.company_id)
    else:
...
async def list_lexware_exports(limit: int = Query(20, ge=1, le=100),
    ...
    jobs = service.list_jobs(str(current_user.company_id), limit)
    return [LexwareExportResponse(

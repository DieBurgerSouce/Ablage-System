# -*- coding: utf-8 -*-
"""
Qwen2.5-VL-7B OCR Agent fuer Ablage-System.

GPT-4o-Level Vision-Language Model optimiert fuer deutsche Dokumente.
Basierend auf docs/OCR_EVALUATION_2025.md Evaluation.

VRAM: ~14GB (knapp unter RTX 4080 16GB Limit)
Staerken: JSON/Struktur-Extraktion, multimodal, hohe Genauigkeit
"""
...
class QwenOCRAgent(OCRAgent):
    """
    Qwen2.5-VL-7B OCR Agent fuer GPT-4o-Level Performance.

    Verwendet das Qwen/Qwen2.5-VL-7B-Instruct Modell fuer hochpraezise
    Textextraktion aus deutschen Geschaeftsdokumenten.
    """
    ...
    def __init__(self):
        """Initialisiere Qwen OCR Agent mit GPU-Optimierungen."""
        # Class-level Lock initialisieren
        if QwenOCRAgent._model_lock is None:
            QwenOCRAgent._model_lock = asyncio.Lock()
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                "qwen_agent_gpu_detected",
                device=torch.cuda.get_device_name(0),
                cuda_version=torch.version.cuda,
                vram_gb=round(torch.cuda.get_device_properties(0).total_memory / 1024**3, 1)
            )
        else:
            logger.warning("qwen_agent_no_gpu_cpu_fallback")

        logger.info(
            "qwen_ocr_agent_initialized",
    ...
    async def _load_models_async(self, timeout_seconds: float = MODEL_LOADING_TIMEOUT):
        """
        Lade Qwen2.5-VL Modell mit Thread-Safe Locking und Timeout.
        ...
        """
        async with QwenOCRAgent._model_lock:
    ...
    def _load_models_sync(self):
        ...
        try:
            ...
            # Importiere Qwen-spezifische Klassen
            # Qwen2.5-VL verwendet Qwen2_5_VLForConditionalGeneration (nicht Qwen2VL!)
            ...
            # Lade Processor
            logger.info("qwen_loading_processor")
            ...
            logger.info("qwen_loading_model", dtype=str(self.dtype))
            self._model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
                self.MODEL_NAME,
                dtype=self.dtype,  # torch_dtype is deprecated, use dtype
            ...
            logger.info("qwen_models_loaded_successfully")
    ...
    def _warmup_model(self):
        ...
        try:
            ...
            logger.info("qwen_warmup_starting")
    ...
    def _load_image(self, image_path: str) -> List[Image.Image]:
        ...
        if path.suffix.lower() == '.pdf':
            # PDF-Verarbeitung
            logger.info("qwen_processing_pdf", path=image_path)
            try:
            ...
            logger.info("qwen_processing_image", path=image_path)
            try:
    ...
    def _process_single_image(self, image: Image.Image, language: str = "de") -> Dict[str, Any]:
        """
        Verarbeite ein einzelnes Bild mit Qwen2.5-VL.
        ...
        try:
            # Nachricht fuer Qwen Vision-Language Model
            messages = [
                {
                    "role": "user",
                    "content": [
            ...
            if found_german:
                logger.debug("qwen_german_chars_detected", chars=found_german)
            ...
            return {
                "text": extracted_text,
                "confidence": 0.95,  # Qwen hat keine native Confidence-Ausgabe
    ...
    async def process(
        ...
        **kwargs
        ...
        """
        Verarbeite Dokument mit Qwen2.5-VL OCR.
        ...
        try:
            ...
            logger.info("qwen_pages_loaded", count=len(images))
            ...
            for i, image in enumerate(images):
                logger.info("qwen_processing_page", page=i + 1, total=len(images))
    ...
    async def cleanup(self):
        ...
        logger.info("qwen_cleanup_starting")
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.synchronize()
            logger.info("qwen_gpu_memory_cleared")

        await super().cleanup()
        logger.info("qwen_cleanup_completed")

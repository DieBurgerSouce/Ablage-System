...
describe('RAG useChatWebSocket Hook - Token Storage', () => {
  beforeEach(() => {
  ...
  it('sollte Token aus sessionStorage lesen', () => {
    sessionStorage.setItem('auth_token', 'test-rag-ws-token');
    expect(sessionStorage.getItem('auth_token')).toBe('test-rag-ws-token');
  });

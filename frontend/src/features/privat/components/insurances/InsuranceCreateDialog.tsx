/**
 * InsuranceCreateDialog - Dialog zum Erstellen einer neuen Versicherung
 */
...
import type { PrivatInsuranceCreate, InsuranceType } from '@/types/privat';

interface InsuranceCreateDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: PrivatInsuranceCreate) => Promise<void>;
  isLoading?: boolean;
}
...
export function InsuranceCreateDialog({
  ...
  const [formData, setFormData] = React.useState<PrivatInsuranceCreate>({
    name: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({
        ...
        coverageAmount: formData.coverageAmount || undefined,
        deductible: formData.deductible || undefined,
        cancellationPeriod: formData.cancellationPeriod || undefined,
      });
      resetForm();
      onOpenChange(false);
    } catch (err) {
      ...
      <DialogContent className="sm:max-w-[525px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Erfassen Sie eine neue Versicherung in Ihrem Portfolio.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {/* Cancellation Period & Auto Renewal */}
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
...
export default InsuranceCreateDialog;

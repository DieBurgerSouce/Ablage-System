/**
 * Loki Push Client fuer Frontend-Logging.
 *
 * Sendet strukturierte Logs an Grafana Loki fuer zentrales Monitoring.
 * Features:
 * - Batch-Sending (sammelt Logs, sendet alle 5s oder bei 10 Eintraegen)
 * - Retry-Logik bei Netzwerkfehlern
 * - User-Kontext fuer bessere Korrelation
...
class LokiClient {
  ...
  /**
   * Synchroner Flush mit sendBeacon fuer beforeunload.
   ...
   * WICHTIG: Keine PII (E-Mail etc.) uebergeben!
   */
  ...
  /**
   * Setzt zusaetzliche Labels fuer alle folgenden Logs.
   */
   ...
   * Wichtig fuer Hot-Reloading und Tests.
   */
  destroy(): void {
    ...
    // Sync-Flush fuer verbleibende Logs
    this.flushSync();
  }
  ...
  push(level: LogLevel, message: string, context?: Record<string, unknown>): void {
    ...
    const entry: LogEntry = {
      timestamp: Date.now().toString() + '000000', // Nanosekunden-Format fuer Loki
      level,
      message,
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      // Bei Fehler: Eintraege zurueck in Buffer (am Anfang)
      // Aber NUR wenn Buffer nicht zwischenzeitlich gefuellt wurde
      this.buffer = [...entries, ...this.buffer];

      // Begrenze Buffer-Groesse um Memory Leaks zu vermeiden
  ...
  /**
   * Erstellt einen neuen Client mit zusaetzlichen Labels.
  ...
  private enrichContext(context?: Record<string, unknown>): Record<string, unknown> {
    ...
    // Tenant-Kontext fuer Multi-Mandanten-RLS
    if (this.tenant) {
  ...
  private async sendToLoki(entries: LogEntry[], retryCount = 0): Promise<void> {
    ...
    try {
      const response = await fetch(`${LOKI_URL}/loki/api/v1/push`, {
        ...
        keepalive: true, // Wichtig fuer beforeunload
      });
  ...
  /**
   * Baut das Loki-Payload fuer sendBeacon.
   * Wiederverwendbar fuer sync und async Sends.
   */
...
/**
 * Helper-Klasse fuer Component-spezifisches Logging.
 */
...
// Type-Export fuer externe Verwendung

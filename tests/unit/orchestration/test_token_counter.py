...
class TestTokenCounter:
    ...
    def test_prose_content_type(self):
        ...
        tokens = counter.count_tokens(prose, content_type=ContentType.PROSE_ENGLISH)
...
'''
        ...
        tokens = counter.count_tokens(code, content_type=ContentType.CODE_PYTHON)
...
'''
    ...
    def test_german_text_efficiency(self):
        ...
        german_tokens = counter.count_tokens(german, content_type=ContentType.PROSE_GERMAN)
        english_tokens = counter.count_tokens(english, content_type=ContentType.PROSE_ENGLISH)
    ...
    def test_special_characters_overhead(self):
        """Special characters should be counted appropriately."""
        counter = TokenCounter()
        simple = "a" * 100
        complex = "!@#$%^&*()" * 10
        ...
        # Both should have reasonable token counts for 100 chars
        # Exact comparison depends on implementation details
        assert simple_tokens > 15  # At least some tokens
        assert complex_tokens > 15  # At least some tokens
    ...
    def test_accuracy_within_target_range(self):
        ...
        for text, expected in test_cases:
            estimated = counter.count_tokens(text, content_type=ContentType.PROSE_ENGLISH)
            # Allow wider margin since exact token count varies
            error_pct = abs(estimated - expected) / expected if expected > 0 else 0
            assert error_pct <= 0.50, f"Error too high for '{text}': {error_pct:.1%} (got {estimated}, expected ~{expected})"
...
'''
        ...
        tokens = counter.count_tokens(indented_code, content_type=ContentType.CODE_PYTHON)
    ...
    def test_content_type_enum_values(self):
        ...
        # Check for actual enum values (PROSE_ENGLISH, CODE_PYTHON, etc.)
        assert hasattr(ContentType, 'PROSE_ENGLISH')
        assert hasattr(ContentType, 'PROSE_GERMAN')
        assert hasattr(ContentType, 'CODE_PYTHON')
    ...
    def test_very_long_text_performance(self):
        ...
        import time
        start = time.time()
        tokens = counter.count_tokens(long_text, content_type=ContentType.PROSE_ENGLISH)
    ...
    def test_markdown_formatting_overhead(self):
        ...
        plain_tokens = counter.count_tokens(plain, content_type=ContentType.PROSE_ENGLISH)
...
'''
        ...
        tokens = counter.count_tokens(code_with_comments, content_type=ContentType.CODE_PYTHON)

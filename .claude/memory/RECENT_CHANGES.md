...
- **Offen (nächste Welle)**: **1a** Multi-Tenant-Default-Isolation (Cross-Engine-`get_db`-Loch bewiesen: **241** Endpoints nutzen `app.api.dependencies.get_db`, aber `require_company` setzt RLS auf `app.db.database.get_db` — 6 Endpoints → RLS auf falscher Session; braucht `get_tenant_db` + RLS-GUC-Konsolidierung + DB-RLS-Regressionstest), **0c** Playwright ehrlich, **0d-CI** Schemathesis-Gate, **0e** Agent-Harness-PoC, **2b** Branch-Hygiene.

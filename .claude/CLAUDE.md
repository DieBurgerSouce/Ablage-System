...
**Status**: 🟡 Substanziell gehärtet, noch NICHT Full Production-Ready (Stand 2026-06-20, gegen Code/Git verifiziert). **master = `43971d7ae`, alembic head 270.** Historische Blocker **B1–B4 behoben/gescoped** (B2: PSD2-OAuth2-Token outscoped, BaFin). **Auf master erledigt** (verifiziert): PaymentService company-scope (Mig 269), RLS_ENFORCE_DEFAULT (fail-closed, opt-in), GoBD-signiertes-Verfahrensdoku-PDF (Mig 270), OTEL-SDK, `build:strict`-CI-Gate, Schemathesis-5xx-Validierung, 192→0 GET-500-Sweep. **Aktive Härtung** auf `fix/pilot-hardening-2026-06-20` (Plan `~/.claude/plans/ethereal-launching-harp.md`): auto_filing/folder_documents, unit-Collection (0→18.763 sammelbar), Bandit-Gate, OCR-Alerts, Audit-Logging, SLO-Doku. **Genuin offen:** DoD = 2 saubere A-Z-Loop-Runs (braucht dediziertes RAM-Fenster + Worker-Stop), KOSIT-Validator/TSE-Klärung (Compliance), Vault-Rotation/K8s-Härtung (Infra), Doku-Staleness, OPEN-30-E2E-Reaktivierung (App-Code gefixt, Specs noch `fixme`). _Frühere Angaben „4 offene Blocker"/„Production-Ready 2026-01-10" überholt._ Details: `.claude/memory/pilot-hardening-2026-06-20.md`, `.claude/reviews/2026-06-11/WAVE1_EXPLORE_REGISTER.md`, `.claude/memory/KNOWN_ISSUES.md`
**Version**: 1.2

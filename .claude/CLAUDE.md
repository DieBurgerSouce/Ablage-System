...
**Status**: 🟡 Substanziell gehärtet, Pilot-Härtung in master gemergt (Stand 2026-06-21, gegen Code/Git verifiziert). **master enthält die Pilot-Hardening-Offensive** (Merge `dc5ad466a` von `fix/pilot-hardening-2026-06-20`, 22 Commits, **inzwischen gepusht — origin/master @ `5aed97ea8` (Stand 2026-06-25)**, Backup-Tag `pre-merge-2026-06-21`). **DoD: A-Z-Loop best-effort grün** — build/up/gates/docker/**unit/security/integ** grün im echten Loop (mit Seed); api-5xx-Sweep 116 Min sauber (0 Server-Fehler = zugesagte Scope); e2e-Reaktivierungen (OPEN-30) + a11y/vitest grün. **Neu in master:** Compliance (Art.30-Verarbeitungsverzeichnis-Endpoint, ZUGFeRD-E-Mail-Auto-Extraktion), GPU-Thermal-Alerts exporter-robust, Repo-Hygiene (16 Branches geprunt), GPU-Mock-Collection-Fix. Historische Blocker **B1–B4 behoben/gescoped** (B2: PSD2-Token outscoped); Mig 269/270, RLS_ENFORCE_DEFAULT, OTEL, build:strict, 192→0 GET-500 (bereits vor-Merge auf master). **Best-effort-Caveats (pre-existierend, dokumentiert, NICHT aus dieser Arbeit):** ① api-Hang/DoS — **geguarded + auf master gemergt 2026-06-22** (ReDoS-Guard `safe_regex` in 4 user-Pattern-Evaluatoren + chunking-step-guard + OCR-Vorschau-Caps + Schemathesis-`--request-timeout`; 10×safe_regex + 4×OCR-Tests grün im Wegwerf-Container; Merges `59d8217d5`+`966303ec0`). Empirischer End-to-End-Re-Fuzz bewusst deferred (Unit-Beweis akzeptiert, braucht exkl. Stack-Fenster); ② e2e 3 Reds — Perf-4-Worker-Contention auf Single-Uvicorn + 2 pre-existierend (error-state-UI, pwa-sw); ③ monitor CeleryNoActivity — wedged solo-GPU-Worker ohne GPU im Dev (Backlog: CPU-OCR-Fallback). **Genuin offen:** formale 2× clean A-Z-Runs, api-Hang-Re-Fuzz (empirisch, deferred — Code-Fix erledigt), KOSIT/TSE (Compliance), Vault-Rotation/K8s. **Erledigt 2026-06-25:** Frontend-Art.30-View (OPEN-43, Merge `5aed97ea8`, `tsc -b` 0 Fehler), `git push origin master` (gepusht), Infra-Alerts via `promtool check` verifiziert sauber (15 Regel-Dateien) + externe Images alle gepinnt, Dependabot 19 PRs triagiert (User-Merge ausstehend). Deferred-mit-Begründung: OPEN-51 lokale `:latest` (Reproduzierbarkeit, kein Supply-Chain-Risiko), OPEN-52 watchdog-Privileg (load-bearing), fremde Worktrees (Parallel-Session aktiv). _Frühere Angaben „4 offene Blocker"/„Production-Ready 2026-01-10" überholt._ Details: `docs/qa-reports/2026-06-25-offene-punkte-statusplan.html` (verbindliches Offene-Punkte-Register), `.claude/memory/KNOWN_ISSUES.md`
**Version**: 1.2
